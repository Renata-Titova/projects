...
for root, dirs, files in os.walk(folder_path):
    for file in files:
        ...
        if file.endswith("seg_B2"):
            files_seg_B2.append(os.path.join(root, file))
...
for filename_y1, filename_b2 in zip(
    files_seg_Y1, files_seg_B2
    ...
    params, sounds = read_seg(filename_b2)
    # print(sounds)

    # Создаем словари для каждого слова
    for left, right in zip(words, words[1:]): # идем по уровню слов
        ...
        corresponding_sounds = [
            sound["name"]
            for sound in sounds
            if start_time <= sound["position"] / params["SAMPLING_FREQ"] < end_time # если метка звука >= левой метки слова но меньше правой метки то записываем их вместе
        ]
        ...
        sound_dict = {
            ...
            "from": start_time,
            "to": end_time,
            "ideal_transcription": result_string,
        }
...
cursor.execute(
    """
    CREATE TABLE IF NOT EXISTS Ideal_transcription (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        filename INTEGER,
        unit TEXT,
        from_time REAL,
        to_time REAL,
        CONSTRAINT FK_Ideal_transcription_Real_transcription FOREIGN KEY (filename) REFERENCES Real_transcription(filename)
        CONSTRAINT FK_Ideal_transcription_Words FOREIGN KEY (filename) REFERENCES Words(filename)
        CONSTRAINT FK_Ideal_transcription_Sintagmas FOREIGN KEY (filename) REFERENCES Sintagmas(filename)
        CONSTRAINT FK_Ideal_transcription_filenames FOREIGN KEY (filename) REFERENCES filenames(filename)
...
for i, sound_dict in enumerate(result):  # Используем enumerate для получения индекса
    ...
    unit = sound_dict["ideal_transcription"]
    from_time = sound_dict["from"]
    to_time = sound_dict["to"]
    formatted_id = i + 4001  # со 4001
    cursor.execute(
        "INSERT INTO Ideal_transcription (id, filename, unit, from_time, to_time) VALUES (?, ?, ?, ?, ?)",
...
cursor.execute("SELECT * FROM Ideal_transcription")
rows = cursor.fetchall()
print("Таблица ideal_transcription:")
for row in rows:
    print(row)

# 5.  Сохранение таблицы Words в CSV-файл
df = pd.DataFrame(rows, columns=["id", "filename", "unit", "from_time", "to_time"])
df.to_csv(os.path.join(folder_path, "ideal_transcription.csv"), index=False, sep=",")
